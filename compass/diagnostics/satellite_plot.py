...
def main():
    ...
    def load_satpy_scene(data_dir, reader_hint=None):
        try:
            files = find_files_and_readers(base_dir=data_dir, reader=reader_hint)
            if not files:
                raise ValueError(f"No Satpy-compatible files found in {data_dir}")
            #scn = Scene(filenames=files, reader=readers)
            return files
